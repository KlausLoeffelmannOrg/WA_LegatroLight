...
**⚙️ Config and App-wide HighDPI settings:** *app.config* files are discouraged for configuration for .NET.
For setting HighDpiMode, use e.g. `Application.SetHighDpiMode(HighDpiMode.SystemAware)` at app startup, don't use *app.config* nor *manifest* files.

Note: `SystemAware` is default for .NET, use `PerMonitorV2` when explicitly requested.
...
**Decision:** In *.designer.cs* or `InitializeComponent` → Designer rules. 
...
### 🛑 For large and XXL Forms: Anti-Refactoring Rules

⚠️ **YOU WILL BE TEMPTED** to create helper methods when generating large forms. 
    **RESIST THIS URGE.**

**Why repetitive code in `.designer.cs` for `InitializeComponent` is CORRECT:**
- The Visual Studio WinForms Designer is a **parser tool**, not a C# compiler
- It uses code in `InitializeComponent` as serialized design data, not C# code
- Therefore, repetitive patterns are **serialization data**, not code smells
- Helper methods make files **uneditable** in the Designer
- `InitializeComponent` code still executes at runtime.
...
- If Forms exceed 15K Tokens --> separate areas out into UserControls and compose Forms from them.
