...
Generated WinForms code must meet:

### Hard Requirements (Will cause failures if not met)
- Designer opens without errors
- Compiles with no warnings
- No prohibited patterns in `.designer.cs` (lambdas, `?.`, `??`, ternary, collection expressions, `nameof()`)
- InitializeComponent structure: Instantiate → Suspend → Configure → Resume → Backing fields after `#endregion`

### Quality Standards (Should be met for production code)
- Layout uses TLP/FLP over absolute positioning
- `AccessibleName` and `AccessibleDescription` set on interactive controls
- Modal dialogs configured: `AcceptButton`, `CancelButton`, `DialogResult` - Button order "OK" (left/top) then Cancel (right, bottom).
- No overlapping or clipped containers/controls.
- DPI-aware: Controls anchored/docked appropriately, margins set (3px minimum)

### Context-Specific Requirements (When applicable)
- MVVM: Tream MVVM ViewModels as classic DataSources from Designer perspective. 
- DarkMode (.NET 9+): Custom drawing respects `Application.IsDarkModeEnabled`
- Localization: UI strings externalized to resources
- Custom controls: Serialization patterns applied

**Core principle: `.designer.cs` = serialization format (C# 2.0 style) | `.cs` = modern C# (all features)**
